...
def get_pair_strategy_return(testing_start_index,testing_end_index):
    ...
    tests.insert(len(tests.columns), 'rbtc_ret(-1)', rbtc_ret.shift(1))
    tests.insert(len(tests.columns), 'rbtc_ret(-2)', rbtc_ret.shift(2))
    tests.insert(len(tests.columns), 'reth_ret(-1)', reth_ret.shift(1))
    tests.insert(len(tests.columns), 'reth_ret(-2)', reth_ret.shift(2))
    tests.insert(len(tests.columns), 'rrbtc(-1)', rrbtc.shift(1))
    tests.insert(len(tests.columns), 'rrbtc(-2)', rrbtc.shift(2))
    tests.insert(len(tests.columns), 'rreth(-1)', rreth.shift(1))
    tests.insert(len(tests.columns), 'rreth(-2)', rreth.shift(2))
