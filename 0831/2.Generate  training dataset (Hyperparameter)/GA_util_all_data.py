...
def triple_barrier_change_rate(zscore,price1_change_rate,price2_change_rate, ub, lb, max_period):
    def end_price(s):
        return np.append(s[(s / s[0] > ub) | (s / s[0] < lb)], s[-1])[0] / s[0]
    ...
    p = zscore.rolling(max_period).apply(end_price, raw=True).shift(-max_period + 1)
    t = zscore.rolling(max_period).apply(end_time, raw=True).shift(-max_period + 1)
    t = pd.Series([t.index[int(k + i)] if not math.isnan(k + i) else np.datetime64('NaT')for i, k in enumerate(t)], index=t.index).dropna()

    signal = pd.Series(0, p.index)
    signal.loc[p > ub] = 1
    signal.loc[p < lb] = -1

    df = pd.DataFrame({'price1_change_rate': price1_change_rate,
                       'price2_change_rate': price2_change_rate,
                       'signal': signal})

    # In a bear market, no trading occurs if both cryptocurrencies fall at the same time.
    df.loc[(df['price1_change_rate'] < 0) & (df['price2_change_rate'] < 0), 'signal'] = 0
    # df.loc[(df['price2_change_rate'] < 0) & (df['signal'] == 1), 'signal'] = 0
    # df.loc[(df['price1_change_rate'] < 0) & (df['signal'] == -1), 'signal'] = 0
    updated_signal = df['signal']

    ret = pd.DataFrame({'triple_barrier_profit': p, 'triple_barrier_sell_time': t, 'triple_barrier_signal': updated_signal})

    # ret.loc[(ret['triple_barrier_profit'] < 0) , 'triple_barrier_signal'] = 0
