...
final_label_path = "0618_y_prediction.csv"
...
plt.plot(predict_port_outa_for_lable, label='Machine Learning Prediction',color='r')
...
plt.title('------------Purposed Approach VS. P-Trading Strategy Cumulative Returns------------')
plt.xlabel("Date")
...
results2 = [{'0': 'Test:',
            ...
            {'0': 'Sharpe Ratio (Rf=0%)',
             '1': np.round(port_outa_pair_trading.iloc[-1] / (np.std(port_outa_pair_trading)), 4),
             '2': np.round(predict_port_outa_for_lable.iloc[-1] / (np.std(predict_port_outa_for_lable)), 4),
             '3': np.round(bh_btc_test.iloc[-1] / (np.std(bh_btc_test)), 4),
             '4': np.round(bh_eth_test.iloc[-1] / (np.std(bh_eth_test)), 4),
             # '6': np.round(pt_outc.iloc[-1] / (np.std(pt_outc)), 4)
             },

            {'0': 'Max Drawdown',
             '1': np.round(get_mdd(port_outa_pair_trading), 4),
             '2': np.round(get_mdd(predict_port_outa_for_lable), 4),
             '3': np.round(get_mdd(bh_btc_test), 4),
             '4': np.round(get_mdd(bh_eth_test), 4),
